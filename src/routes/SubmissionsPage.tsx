        ...
        if (isAdmin) {
          ...
          const mapped = pubs.map((p: any) => {
            ...
            return {
              ...
              subtitle: p.subtitle || "",
              fileCount,
              publisherDomain: p.shortId ? "scienteen.com" : "press.openrockets.com",
              linkId: p.shortId ? p.shortId : `artifacts/${(p.title || "").toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')}-${p.pubId || p.id}`,
              hashtags: p.tags?.map ? p.tags.map((t: string) => ({ id: t, name: t, type: "general" as const })) : [],
              author: p.authorName || "Unknown Author",
              status: p.status,
              createdAt: p.submittedAt ? new Date(p.submittedAt).getTime() : Date.now()
